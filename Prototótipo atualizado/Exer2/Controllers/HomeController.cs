    ...
    public class HomeController : Controller
    {
        public class UsuarioDAL
        {
            public static bool VerificaSenha(string SENHA)
            {
                ...
                {
                    var existeSENHA = (from u in dc.Usuario
                                       where u.Senha == SENHA
        ...
        public ActionResult Login(Usuario u)
        {
            ...
            if (ModelState.IsValid) //verifica se é válido
            {
                using(CEPAD_IIIEntities dc = new CEPAD_IIIEntities())
                {
                    var v = dc.Usuario.Where(a => a.USuario1.Equals(u.USuario1) && a.Senha.Equals(u.Senha)).FirstOrDefault();
                    if (v != null)
                    {
                        Session["usuarioLogadoID"] = v.ID_Usuario.ToString();
                        Session["nomeUsuarioLogado"] = v.USuario1.ToString();
                        return RedirectToAction("Index", "Municipes");
                    }
                }
            }
            return View(u);
